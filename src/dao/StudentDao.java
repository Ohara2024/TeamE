...
public class StudentDao extends Dao {
    ...
    /**
     * filterメソッド 学校コードと在学フラグを指定して学生の一覧を取得する
     *
     * @param school:School 学校
     * @param isAttend:boolean 在学フラグ
     * @return 学生のリスト:List<Student> 存在しない場合は0件のリスト
    ...
    /**
     * findByConditionsメソッド 任意の条件で検索する
     *
     * @param school:School 学校
     * @param entYear:Integer 入学年度（null可）
     * @param classNum:String クラス番号（nullまたは空文字可）
     * @param isAttend:Boolean 在学フラグ（null可）
     * @return 学生のリスト List<Student>
     * @throws Exception
     */
    public List<Student> findByConditions(School school, Integer entYear, String classNum, Boolean isAttend) throws Exception {
        List<Student> list = new ArrayList<>();
        Connection connection = getConnection();
        PreparedStatement statement = null;
        ResultSet rSet = null;
        ...
        try {
            ...
            int index = 1;
            statement.setString(index++, school.getCd());
            if (entYear != null) {
